...
private:
    ...
    // MAX: -2e18 (-INF)
    // LAZY_ADD: 0
    // LAZY_SET: 2e18 (Or any value outside possible input range)
    ...
    // MAX:  return max(a, b);
    // XOR:  return a ^ b;
    // PROD: return a * b; // (Use (a * b) % MOD if needed)
    // GCD:  return std::gcd(a, b);
    // ALT:  return (level % 2 == 0) ? (a ^ b) : (a | b); // Level-aware
...
public:
    ...
    // Range Update. (For Point Update, just pass the same index for l and r)
